...
Pack and multi-pack-index validation is reused independently for every object database. If an alternate contains a corrupt copy and a later alternate has an independent valid copy, reads may recover from the valid copy. Corruption in the **primary** store remains a hard error and is not hidden by borrowing.
...
- packed-only alternate objects through the alternate's MIDX;
