# Phase 274 — annotated-tag-aware `rev-list object:type` filtering

Phase274 closes the long-standing annotated-tag object traversal gap in the
...
not present in that inventory, so non-ordered `object:type=tag` was rejected and
existing `commit|tree|blob` filters also missed Git's provided-tag exemption.

Phase274 adds a small composition adapter that:
...
Annotated tag objects are ordinary local Git objects in pygit's object store.
Every emitted commit/tag/tree/blob identity is therefore a genuine 64-hex local
SHA-256 object ID. No foreign transport SHA-1 is padded, translated, or
promoted into repository-visible output.
...
This phase is deliberately non-ordered line/count support. Annotated-tag
placement is still rejected here for:

- structured `-z` output;
- `--in-commit-order` output;
- `--disk-usage`.

Concurrent Phase273 / PR #251 independently implements an ordered annotated-tag
root slice. Phase274 does not overwrite or depend on that sibling. A later
consolidation phase can combine the two once both exact heads are green.
...
- Phase272 became independent PR #250 based on Phase270 and was not modified;
- while this work was in progress, another worker created Phase273 / PR #251
  for ordered tag roots;
- this generic non-ordered work was therefore cleanly renumbered to Phase274
  rather than competing for the same phase number or overwriting PR #251;
- Phase274 was free immediately before the clean renumber branch was created.

## Verification

Focused tests cover explicit annotated tag roots, nested chains, tag-name
presentation, `--no-object-names`, `--filter-provided-objects`, counts, `--all`
deduplication, peeled `^{}` expressions, empty object:type omission sets,
provided-tag behavior for existing commit/tree/blob filters, explicit
NUL/ordered/disk-usage follow-up guards, and native Git 2.55 SHA-256 behavior.
