...
Phase 56 adds an index-to-worktree plumbing path that complements `update-index`, `ls-files`, and `read-tree`. Phase 125 extends that path to the multi-stage conflict index introduced in Phase 124, so base/ours/theirs blobs can be inspected without resolving the conflict.
...
The default `stage=0` behavior is intentionally unchanged from Phase 56. Phase 125 supports selecting one stage at a time; Git's `checkout-index --stage=all --temp` workflow is outside this phase because pygit does not yet expose checkout-index temporary-file mapping output.
