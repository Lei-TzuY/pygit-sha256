# Phase 66: hardened pack import plumbing

Phase 66 adds low-level import operations for pygit's native SHA-256 pack format. It complements the existing `repack`, `verify-pack`, `count-objects`, and `fsck` commands with the inverse operations needed to reconstruct an index or materialize packed objects as loose storage.
...
The standalone parser does not trust or require an existing `.idx`. It walks the pack stream sequentially and checks the `PACK` signature/version/count, bounded size varints, supported commit/tree/blob/tag type IDs, complete zlib streams, pack/envelope sizes, canonical object envelopes, typed object structure, SHA-256 object identity, duplicate IDs, CRC-32 values, exact stream termination, and the final SHA-256 pack trailer.

Zlib decoding is bounded to the size declared by each pack entry plus one sentinel byte. An under-declared compressed member is therefore rejected as soon as it expands beyond the claimed size instead of first materializing an arbitrarily large decompressed byte string. Canonical-envelope checking also rejects alternate encodings such as padded size fields, and the existing typed `hash-object` validator rejects structurally incomplete commit/tree/tag payloads.

A damaged, truncated, oversized, or structurally malformed pack therefore fails before `unpack-objects` starts creating loose objects.
