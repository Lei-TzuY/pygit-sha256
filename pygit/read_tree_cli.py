...
def run_read_tree(argv: Sequence[str]) -> int:
    ...
    if args.merge:
        ...
        if args.update:
            parser.error("Phase 127 does not combine -m with -u")
    ...
    # Phase 124 introduced persistent conflict stages. A replacing read-tree or
    # --empty operation must discard them together with the old stage-0 index.
    # Snapshot the in-memory side table so validation failures leave the caller
    # with the pre-command state; the original on-disk index is not published
    # until read_tree() itself succeeds.
