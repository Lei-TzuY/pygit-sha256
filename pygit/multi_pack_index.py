...
class ParsedMultiPackIndex:
    ...
    def lookup(self, oid: str) -> Optional[MultiPackIndexEntry]:
        ...
        # Keep lookup O(log N) on Python 3.9 without relying on the ``key=``
        # parameter added to :mod:`bisect` in newer Python releases.
        low = 0
        high = len(self.entries)
        while low < high:
            middle = (low + high) // 2
            candidate = self.entries[middle].oid
            if candidate < oid:
                low = middle + 1
            else:
                high = middle
        if low < len(self.entries) and self.entries[low].oid == oid:
...
def _build_bytes(
    ...
    ooff = b"".join(
        struct.pack(">II", selected[oid][0], selected[oid][1]) for oid in oids
    )
...
def parse_multi_pack_index_bytes(data: bytes) -> ParsedMultiPackIndex:
    ...
    for index, oid in enumerate(oids):
        pack_id, offset = struct.unpack(
            ">II",
            ooff[
                index * _OFFSET_RECORD_SIZE : (index + 1) * _OFFSET_RECORD_SIZE
            ],
        )
