...
``objects/pack``. Phase 122 also supports Git-style
...
class ObjectStore:
    ...
    def write(self, obj: GitObject) -> str:
        ...
        fd, temp_name = tempfile.mkstemp(prefix=f".tmp-{sha}-", dir=str(obj_path.parent))
    ...
    def read_store_bytes(self, sha: str) -> bytes:
        """Return exact validated store bytes from primary or alternates.
        ...
        if midx is not None:
            ...
            if mapping is not None:
                ...
                if not idx_file.is_file() or not idx_file.with_suffix(".pack").is_file():
                    raise ValueError(f"multi-pack-index references missing pack {mapping.pack_name}")
    ...
    def _parse(store_bytes: bytes) -> GitObject:
        ...
        if len(payload) != declared_size:
            raise ValueError(f"Size mismatch: header says {declared_size}, got {len(payload)}")
