...
``objects/pack``. Phase 123 also supports Git-style
...
class ObjectStore:
    ...
    Alternate object databases extend only the visible read set. ``write()``,
    ``write_raw()``, and ``delete()`` always target ``self.root`` so a borrower
    cannot mutate object stores it does not own.
    """

    def __init__(self, objects_dir: Path) -> None:
        self.root = Path(objects_dir)
        self.root.mkdir(parents=True, exist_ok=True)
        # MIDX caches are independent for the primary and every alternate root.
    ...
    def write(self, obj: GitObject) -> str:
        """Serialize *obj* and atomically publish it in the primary store."""
    ...
    def storage_roots(self) -> Tuple[Path, ...]:
        ...
        Alternate discovery is reevaluated on every call so an edited
        ``objects/info/alternates`` file becomes visible without reconstructing
        the Repository/ObjectStore instance.
        """
    ...
    def read_store_bytes(self, sha: str) -> bytes:
        ...
        The primary database is authoritative: corruption there is surfaced
        immediately instead of being hidden by an alternate. A primary miss
        falls through to alternates in configured/transitive order. Damaged
        alternate copies may fall through to later independent copies.
        """
        primary = self.root.resolve()
        try:
            return self._read_store_bytes_local(primary, sha)
        except KeyError:
            pass
        ...
        for root in self.storage_roots()[1:]:
            try:
                return self._read_store_bytes_local(root, sha)
            except KeyError:
                continue
    ...
    def _read_store_bytes_local(self, root: Path, sha: str) -> bytes:
        ...
        if obj_path.exists():
            ...
            # Preserve Phase 119's guarantee that raw reads are still readable
            # pygit objects, while returning the original envelope byte-for-byte.
            self._parse(store_bytes)
            return store_bytes

        return self._read_packed_store_bytes_local(root, sha)

    def _read_packed_store_bytes_local(self, root: Path, sha: str) -> bytes:
        """Read exact stored bytes from packs in one object database only."""
        ...
        # On a healthy MIDX miss, covered indexes cannot contain the OID. If
        # the selected copy failed, scan every other index so redundant copies
        # in the same object database can still recover the read.
