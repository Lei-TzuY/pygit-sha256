...
from other object directories listed in ``objects/info/alternates``.  Pygit
uses the same file location and relative-path rule while keeping its own
SHA-256 object format: every non-empty line names another *pygit* object
folder, and relative paths are resolved from the object database itself.
"""
...
def _direct_alternates(objects_dir: Path) -> Tuple[Path, ...]:
    ...
    Blank lines are ignored.  Paths may be absolute or relative to
    ``objects_dir``.  Missing/non-directory targets fail loudly because an
    alternates file is repository metadata rather than a best-effort hint.
    """
...
def alternate_object_dirs(
    ...
    *,
    ...
    depth limit prevents a malicious or accidentally enormous alternates chain
    from consuming unbounded recursion/IO while still allowing realistic
    shared-object-store layouts.
    """
    if max_depth < 1:
    ...
    def visit(root: Path, depth: int) -> None:
        if depth >= max_depth:
            ...
            unseen = [path for path in direct if path not in seen]
            if unseen:
                raise ValueError(
