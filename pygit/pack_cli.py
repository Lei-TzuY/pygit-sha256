...
def run_index_pack(argv: Sequence[str]) -> int:
    parser = argparse.ArgumentParser(prog="pygit index-pack", description="Validate a pygit SHA-256 pack and create its fan-out index.")
    ...
    if args.verbose:
        ...
        for entry in parse_pack(result.pack_path).entries:
            print(f"{entry.oid} {entry.type_name} {entry.size} {entry.compressed_size} {entry.offset}")
    print(result.idx_path)
    return 0


def run_unpack_objects(argv: Sequence[str]) -> int:
    parser = argparse.ArgumentParser(prog="pygit unpack-objects", description="Validate a pygit SHA-256 pack and materialize loose objects.")
    parser.add_argument("-n", "--dry-run", action="store_true", help="validate only; do not write loose objects")
    parser.add_argument("--strict", action="store_true", help="run fsck integrity checks after unpacking")
    parser.add_argument("pack", metavar="PACK")
    args = parser.parse_args(list(argv))
    repo = _find_repo()
    ...
    if args.strict and not args.dry_run:
        ...
        if report.errors:
            ...
            return 1
    print(f"objects {result.object_count}; written {result.written}; existing {result.existing}")
    return 0
