...
def install_repository_merge_stage_support(repository_cls: Type[object]) -> None:
    """Install Phase 128 merge-stage integration once for *repository_cls*."""
    if getattr(repository_cls, "_phase128_merge_stage_support", False):
        return
    ...
    def _write_merge_state(
        ...
        # Publish the stage model before operation metadata. If construction
        # fails, callers do not observe a half-created MERGE_HEAD.
    ...
    def _clear_merge_state(self) -> None:
        # commit() calls _clear_merge_state even for ordinary commits, so only
        # clear unmerged entries when a real MERGE_HEAD existed.
    ...
    repository_cls._phase128_merge_stage_support = True
