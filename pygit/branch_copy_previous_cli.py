...
def run_branch_copy_previous(argv: Sequence[str]) -> int:
    ...
    source_log = repo.refs._log_path(f"refs/heads/{expanded}")
    ...
    repo.refs._append_reflog(
        ...
        source_oid,
        source_oid,
        f"Branch: copied refs/heads/{expanded} to refs/heads/{destination}",
        force=True,
    )

    _copy_branch_config(repo, expanded, destination)
    return 0
