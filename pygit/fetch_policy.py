...
Phase182 keeps policy decisions separate from the smart-HTTP transport.  The
module resolves CLI/config precedence for pruning and tag behavior and parses
the subset of fetch refspecs used by named remotes, clone mappings, and tag
pruning.
"""
...
def configured_fetch_refspecs(repo: Repository, remote: str) -> List[FetchRefspec]:
    ...
    if not values:
        values = [f"+refs/heads/*:refs/remotes/{remote}/*"]
...
def resolve_fetch_policy(
    ...
    *,
    ...
    resolved_prune = (
        ...
        else _config_bool(repo, remote, "prune", "prune")
    )
    resolved_prune_tags = (
        prune_tags
