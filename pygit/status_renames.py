...
Phase 152 keeps rename detection outside :mod:`pygit.repo` so the historical
...
def parse_similarity_threshold(value: Optional[str]) -> int:
    ...
    if not 0 <= rounded <= 100:
        raise ValueError(f"rename similarity threshold must be between 0% and 100%: {value!r}")
    return rounded
...
def detect_staged_renames(
    ...
    *,
    ...
    Detection is restricted to the HEAD->index side, matching the part of
    ``git status`` where an untracked worktree destination is not itself treated
    as a rename until it has been staged.  Conflicted paths are excluded because
    ...
    # Diffcore performs a global pairing step.  A compact greedy maximum-score
    # selection gives deterministic one-to-one rename pairs and avoids one source
    # or target being reported multiple times.
