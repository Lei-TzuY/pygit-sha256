...
Phase258 extends the existing promisor-aware object-filter stack with Git's
size-limited blob filter.  Local blobs can be classified from their already
materialized payload without network access.  An unresolved promised blob does
not currently carry persistent size metadata in pygit, so this adapter fails
explicitly instead of fetching it merely to decide whether the filter keeps it.
"""
...
def _project(argv: Sequence[str]) -> list[str]:
    ...
    if "--filter-print-omitted" in argv:
        raise ValueError("--filter=blob:limit with --filter-print-omitted is not yet supported")
...
def _local_blob_size(repo, oid: str) -> Optional[int]:
    if len(oid) != 64 or any(ch not in "0123456789abcdef" for ch in oid.lower()):
        return None
    try:
        obj = repo.store.read(oid.lower())
