...
select conflict stages 1 (base), 2 (ours), or 3 (theirs) explicitly. Phase 139
...
def _select_temp_paths(
    ...
    *,
    ...
    if stage == "all":
        ...
        # Native checkout-index silently omits explicitly named stage-0-only
        # paths under --stage=all, but still errors for names absent from the
        # index entirely.  Check the union of stage-zero and unmerged paths to
    ...
    for path in selected:
        ...
        for selected_stage in stages:
            ...
            if entry is None:
                if stage == "all":
                    continue
                _entry_blob(repo, path, selected_stage)  # raises canonical error
