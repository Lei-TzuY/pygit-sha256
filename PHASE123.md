# Phase 123 — Alternate object databases

Phase 123 turns the previously diagnostic-only `objects/info/alternates` file into a real read path. A pygit repository may now borrow SHA-256 loose and packed objects from other pygit object databases without copying them into its own `.pygit/objects` directory.
...
Because native Git and pygit use different repository/object formats, Phase 123 intentionally implements the repository-local `objects/info/alternates` mechanism only; it does not consume Git's process-wide `GIT_ALTERNATE_OBJECT_DIRECTORIES` environment variable.
...
- Phase 120 single-object `cat-file -p`
...
Git's `cat-file --batch-all-objects` includes alternate object databases. Phase 123 applies the same visibility rule:
...
- Phase 120 byte-faithful `cat-file -p` over a borrowed binary blob;
