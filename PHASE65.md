# Phase 65: ls-remote plumbing

Phase 65 adds read-only Smart HTTP remote-reference inspection without fetching a pack or mutating local repository state. It is based on the merged Phase 63 `mktag` baseline and preserves that launcher/API route.
...
This command performs advertisement discovery only. It does not download pack data, write objects, update remote-tracking refs, modify repository remote configuration, or alter HEAD/index/worktree state.
