# Phase 128 — porcelain merge conflict index stages

Phase 128 connects high-level `pygit merge` conflicts to the persistent Git-style multi-stage index introduced in Phase 124, the stage extraction plumbing added in Phase 126, and the write-tree safety checks added in Phase 127.
...
`pygit merge --abort` restores the original HEAD worktree/index and removes the unmerged stage records created for the aborted merge. Successful merge completion similarly leaves no stale stages behind.
...
Phase 128 migrates **merge** conflicts. Cherry-pick and rebase still reuse `_apply_three_way()` but maintain their own operation-state files; migrating those workflows to the same stage model is the next phase.

## Regression coverage

`tests/test_phase128.py` covers real content conflicts producing exact stage 1/2/3 object IDs; revision, `ls-files`, and `checkout-index` visibility; normal add-based resolution and two-parent merge commits; merge abort cleanup; and asymmetric modify/delete conflicts.
