...
The first argument must resolve to an actual ref. The optional second argument is a commit-ish and defaults to `HEAD`. A successful query prints one SHA-256 commit ID and exits 0. If no retained ref tip is a valid fork point, the command prints nothing and exits 1.
...
All ancestry traversal continues to use the existing SHA-256 commit resolver and obey `.pygit/shallow` boundaries. The operation is fully read-only: no refs, reflogs, objects, index entries, or worktree files are changed.
...
point = fork_point(repo, "origin/main", "topic")
if point is None:
