"""Phase410 regressions for ``checkout --detach @{-N}``."""
...
def _seed_repo(path: Path) -> tuple[Repository, str]:
    ...
    base = repo.commit(
        "base",
        author_name="Phase410",
        author_email="phase410@example.invalid",
    )
    repo.refs.set_branch("topic", base, message="branch: topic")
    return repo, base


def test_checkout_detach_previous_branch_uses_sha256_tip(
...
def test_checkout_detach_previous_detached_oid_round_trips(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    repo, base = _seed_repo(tmp_path / "repo")
    capsys.readouterr()
...
def test_application_routes_only_exact_detach_previous_selector(
...
def test_native_git_and_pygit_detach_previous_match_state_and_reflog(
    ...
    assert _git("config", "user.name", "Phase410", cwd=native).returncode == 0
    assert _git("config", "user.email", "phase410@example.invalid", cwd=native).returncode == 0
