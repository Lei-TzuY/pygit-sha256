...
def test_config_change_prunes_stale_client_even_when_no_query_is_needed(
...
def test_changed_server_options_prune_old_effective_configuration(
    tmp_path, monkeypatch
):
    native_a = "4" * 40
    native_b = "5" * 40
...
def test_removed_promisor_remote_is_pruned_without_harming_active_sibling(
    tmp_path, monkeypatch
):
    native = "6" * 40
    repo = _repo(tmp_path, native)
    ...
    assert len(_cached_keys(repo)) == 2
