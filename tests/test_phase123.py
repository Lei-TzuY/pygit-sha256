"""Phase 123 tests: Git-style alternate pygit object databases."""
...
def _set_alternates(repo: Repository, *roots: Path) -> Path:
    ...
    lines = [
        os.path.relpath(Path(root).resolve(), repo.store.root.resolve())
        for root in roots
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def test_relative_alternate_exposes_loose_objects_short_ids_and_raw_bytes(
    tmp_path: Path,
) -> None:
    ...
    assert borrower.store.storage_roots() == (
        borrower.store.root.resolve(),
...
def test_packed_only_alternate_uses_its_multi_pack_index_without_materializing(
    tmp_path: Path,
) -> None:
    source = _repo(tmp_path / "source")
    borrower = _repo(tmp_path / "borrower")
    ...
    assert borrower.store.exists(oid)
    assert borrower.store.read_store_bytes(oid) == obj._build_store_bytes()
    restored = borrower.store.read(oid)
    assert isinstance(restored, BlobObject)
    assert restored.data == b"packed alternate\n"
...
def test_transitive_alternates_are_cycle_safe_and_deduplicated(tmp_path: Path) -> None:
    ...
    assert first.store.storage_roots() == (
        first.store.root.resolve(),
        second.store.root.resolve(),
        third.store.root.resolve(),
    )
...
def test_cat_file_pretty_reads_borrowed_object_byte_exactly(tmp_path: Path) -> None:
    source = _repo(tmp_path / "source")
    borrower = _repo(tmp_path / "borrower")
    payload = b"borrowed\x00binary\xffpayload\n"
    oid = source.store.write(BlobObject(payload))
    _set_alternates(borrower, source.store.root)

    result = _run_cat_file(borrower, "-p", oid)
...
def test_batch_all_objects_includes_alternates_once_in_both_orders(tmp_path: Path) -> None:
    ...
    ordered_oids = [
        ...
        for line in ordered.stdout.splitlines()
    ]
    assert ordered_oids == sorted(expected)

    unordered = _run_cat_file(
        borrower,
        "--batch-check",
        "--batch-all-objects",
        "--unordered",
    ...
    unordered_oids = [
        line.split(b" ", 1)[0].decode("ascii")
        for line in unordered.stdout.splitlines()
    ]
