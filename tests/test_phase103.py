"""Phase 103 tests: SHA-256 multi-pack-index storage and lookup."""
...
def test_object_store_uses_midx_fast_path_for_covered_packs(tmp_path: Path) -> None:
    ...
    corrupt_idx = second_idx if second_idx != first_idx else first_idx
    data = bytearray(corrupt_idx.read_bytes())
    data[-1] ^= 0x01
    corrupt_idx.write_bytes(data)
