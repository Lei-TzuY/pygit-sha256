...
def test_incremental_transaction_publishes_lmap_before_ref_commit(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(phase336, "_download_optional_packfile_uris", lambda *a, **k: EMPTY_BATCH)
...
def test_real_inline_incremental_transaction_makes_new_tip_lookupable(tmp_path: Path):
    repo = Repository.init(str(tmp_path / "repo"))
