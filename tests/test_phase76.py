"""Phase 76 tests: revision-aware, typed ``ls-tree`` plumbing."""
...
def _fixture(repo: Repository) -> dict[str, str]:
    ...
    nested_tree = repo.store.write(TreeObject([TreeEntry("100644", "nested.txt", nested_blob)]))
...
def test_recursive_directory_and_show_tree_modes_are_distinct(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
    _fixture(repo)
    assert [entry.path for entry in ls_tree(repo, recursive=True)] == [
        "file.txt",
        "link",
        "module",
        "run.sh",
    ...
    assert [entry.path for entry in ls_tree(repo, recursive=True, show_trees=True)] == [
        "file.txt",
        "link",
        "module",
        "run.sh",
    ...
    assert [(entry.object_type, entry.path) for entry in directories] == [("tree", "sub")]
...
def test_shared_revision_resolver_handles_tags_short_sha_and_rev_path(tmp_path: Path) -> None:
    ...
    assert [(entry.path, entry.oid) for entry in subtree] == [("nested.txt", ids["nested_blob"])]
...
def test_nested_literal_pathspec_uses_minimum_nonrecursive_traversal(tmp_path: Path) -> None:
    ...
    assert [(entry.path, entry.oid) for entry in selected] == [("sub/nested.txt", ids["nested_blob"])]
    directory = ls_tree(repo, "HEAD", patterns=("sub",))
    assert [(entry.object_type, entry.path) for entry in directory] == [("tree", "sub")]
...
def test_recursive_pathspec_skips_unrelated_broken_subtrees(tmp_path: Path) -> None:
    ...
    wanted_tree = repo.store.write(TreeObject([TreeEntry("100644", "nested.txt", wanted_blob)]))
    not_a_tree = repo.store.write(BlobObject(b"broken child"))
    root = repo.store.write(
        TreeObject(
            [TreeEntry("040000", "broken", not_a_tree), TreeEntry("040000", "wanted", wanted_tree)]
        )
    )
    selected = ls_tree(repo, root, recursive=True, patterns=("wanted/nested.txt",))
    assert [(entry.path, entry.oid) for entry in selected] == [("wanted/nested.txt", wanted_blob)]
...
def test_non_treeish_and_invalid_tree_entries_fail_cleanly(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="not a tree-ish"):
        ls_tree(repo, blob)
    invalid_mode_tree = repo.store.write(TreeObject([TreeEntry("999999", "bad", blob)]))
    with pytest.raises(ValueError, match="unsupported tree entry mode"):
        ls_tree(repo, invalid_mode_tree)
    invalid_name_tree = repo.store.write(TreeObject([TreeEntry("100644", "../bad", blob)]))
