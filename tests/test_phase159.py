"""Phase 159 tests: staged rename detection and porcelain type-2 records."""
...
def test_exact_staged_rename_is_detected_as_r100(tmp_path: Path) -> None:
    ...
    assert records[0].path == "new.txt"
    assert records[0].code == "R "
    assert records[0].orig_path == "old.txt"
    assert records[0].score == 100


def test_long_short_and_porcelain_v1_render_staged_rename(tmp_path: Path) -> None:
    repo = _base_repo(tmp_path)
    _stage_rename(repo)
    ...
    assert "renamed:\told.txt -> new.txt" in long_status.stdout
    assert "deleted:\told.txt" not in long_status.stdout
    assert "new file:\tnew.txt" not in long_status.stdout
...
def test_porcelain_v1_z_uses_target_then_source_nul_order(tmp_path: Path) -> None:
    ...
    implied = _run(repo, "status", "-z")

    assert explicit.returncode == 0, explicit.stderr
    assert explicit.stdout == "R  new.txt\0old.txt\0"
    assert implied.stdout == explicit.stdout
...
def test_no_renames_restores_delete_add_records_in_all_formats(tmp_path: Path) -> None:
    repo = _base_repo(tmp_path)
    _stage_rename(repo)

    long_status = _run(repo, "status", "--no-renames")
    v1 = _run(repo, "status", "--porcelain=v1", "--no-renames")
    v2 = _run(repo, "status", "--porcelain=v2", "--no-renames")

    assert "deleted:\told.txt" in long_status.stdout
    assert "new file:\tnew.txt" in long_status.stdout
    assert "renamed:" not in long_status.stdout
    assert "A  new.txt\n" in v1.stdout
    assert "D  old.txt\n" in v1.stdout
...
def test_unstaged_filesystem_move_is_not_synthesized_as_staged_rename(tmp_path: Path) -> None:
    ...
    matches = detect_staged_renames(repo)
    result = _run(repo, "status", "--porcelain=v1", "-u=all")
...
def test_rename_detection_composes_with_untracked_mode_and_ignored_mode(tmp_path: Path) -> None:
    repo = _base_repo(tmp_path)
    _stage_rename(repo)
    _write(repo, "extra/untracked.txt", "u\n")
    _write(repo, ".gitignore", "ignored/\n")
    _write(repo, "ignored/a.txt", "i\n")

    hidden = _run(repo, "status", "--porcelain=v1", "-u=no")
    ignored = _run(repo, "status", "--porcelain=v2", "--ignored=matching")

    assert "R  old.txt -> new.txt\n" in hidden.stdout
    assert "??" not in hidden.stdout
    assert "2 R. " in ignored.stdout
    assert "! ignored/" in ignored.stdout


def test_rename_paths_follow_v1_quoting_and_z_raw_path_rules(tmp_path: Path) -> None:
    repo = Repository.init(str(tmp_path / "repo"))
    _write(repo, "old\nname.txt", "same\n")
    repo.add(["old\nname.txt"])
    repo.commit("base")
    repo.rm("old\nname.txt")
    _write(repo, "new\tname.txt", "same\n")
    repo.add(["new\tname.txt"])

    text = _run(repo, "status", "--porcelain=v1")
    nul = _run(repo, "status", "--porcelain=v1", "-z")

    assert text.stdout == 'R  "old\\nname.txt" -> "new\\tname.txt"\n'
    assert nul.stdout == "R  new\tname.txt\0old\nname.txt\0"
