"""Phase 120 tests: Git-style alternate pygit object databases."""
...
def test_relative_alternate_exposes_loose_objects_and_short_ids(tmp_path: Path) -> None:
    source = _repo(tmp_path / "source")
    borrower = _repo(tmp_path / "borrower")
    oid = source.store.write(BlobObject(b"borrowed loose object\n"))
    _set_alternates(borrower, source.store.root)

    roots = borrower.store.storage_roots()
    assert roots == (borrower.store.root.resolve(), source.store.root.resolve())
...
def test_packed_only_alternate_uses_its_multi_pack_index(tmp_path: Path) -> None:
    source = _repo(tmp_path / "source")
    borrower = _repo(tmp_path / "borrower")
...
def test_alternate_corruption_can_fall_through_to_later_valid_copy(tmp_path: Path) -> None:
    ...
    # Keep the name but make the first alternate's loose copy invalid.
