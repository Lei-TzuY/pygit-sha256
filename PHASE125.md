# Phase 125 — checkout-index conflict-stage extraction

Phase 125 connects the Phase 56 `checkout-index` plumbing to the persistent multi-stage index introduced in Phase 124. Conflict records can now be materialized directly from stages 1, 2, or 3 without changing `HEAD`, refs, the index, or the conflict state.
...
Phase 125 also does not yet make high-level `merge`, `cherry-pick`, or `rebase` porcelain automatically populate conflict stages. It makes the stage-aware inspection path ready for those workflows once they migrate to the Phase 124 index model.

## Regression coverage

`tests/test_phase125.py` covers base/ours/theirs extraction, stage-filtered `--all`, preservation of historical stage-0 behavior, asymmetric missing-stage errors, index immutability, the focused Python CLI adapter, installed `python -m pygit` routing, and invalid-stage rejection.
