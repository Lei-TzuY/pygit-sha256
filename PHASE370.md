...
3. if that two-hex fanout directory was created by the same write, fsyncing the new directory itself does not make its name durable in `objects/`; the objects root must also be fenced.
...
4. record whether the two-hex fanout directory already exists;
5. create that fanout when needed and create a same-directory temporary inside it;
6. write and flush the zlib-compressed object;
7. durability-fence the file through shared `_fsync_retry(fd)`;
8. atomically replace the final content-addressed pathname;
9. on POSIX, fsync the containing fanout directory through shared `fsync_directory()`;
10. if the fanout was newly created, fsync the `objects/` root so the fanout directory entry itself is durable;
11. return the SHA-256 object id only after all applicable durability fences succeed.
...
A pre-replace durability failure removes the temporary and exposes no new object. A post-replace fanout/root directory-fsync failure propagates even though the complete object may already be visible; the caller is not told that publication durably succeeded.
...
- a new fanout executes file -> fanout directory -> objects-root durability fences;
- a pre-existing fanout skips the unnecessary objects-root fence;
...
- a new-fanout objects-root fsync failure also propagates without falsely reporting durable success;
